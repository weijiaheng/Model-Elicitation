...
def calculate_score(noise_rate, noise_type, model,elicitor, test_dataloader, Loss_zero_one_peer_one_hot, Loss_ce_peer_prob):
    
    for i, data in enumerate(tqdm(test_dataloader), 0):
        inputs, labels = data
        inputs = inputs.to(device)
        
        elicitor_outputs = elicitor(inputs)
        elicitor_outputs = elicitor_outputs.to(device)
        _, labels = elicitor_outputs.max(1)
        
        count = 0
        _, store = elicitor_outputs.max(1)
        for row in labels.split(1):
            labels[count] = add_noise(row, noise_rate, noise_type="Sparse").to(device)
            count += 1
        ...
        peer_dataloader1 = peer_datasets.cifar10(datasize, datasize, IMG_SIZE)
        ...
        peer_dataloader2 = peer_datasets.cifar10(datasize, datasize, IMG_SIZE)
        for k, data_p2 in enumerate(tqdm(peer_dataloader2), 0):
            inputs_p2, labels_p2 = data_p2
            
            inputs_p2 = inputs_p2.to(device)
            elicitor_outputs_p2 = elicitor(inputs_p2)
            elicitor_outputs_p2 = elicitor_outputs_p2.to(device)
            _, labels_p2 = elicitor_outputs_p2.max(1)
            
            count = 0
            _, store = elicitor_outputs_p2.max(1)
            for row in labels_p2.split(1):
                labels_p2[count] = add_noise(row, noise_rate, noise_type="Sparse").to(device)
                count += 1
...
if __name__ == '__main__':
    ...
    noise_type = "Sparse"
    test_dataloader = peer_datasets.load_agent_cifar10(batch_size, test_indices, img_size=(32, 32))
    
    # Choose agent number: 1 or 2, the other one would be the elicitor
    agent_number = 1
    if agent_number == 1:
        model = ResNet34().to(device)
        model_path = './trained_models/cifar10_f1_exp1_CIFAR10_0.5_True_dif_model.pth'
        elicitor = Model2().to(device)
        elicitor_path = './trained_models/cifar10_f2_exp1_CIFAR10_0.5_True_dif_model.pth'
    else:
        model = Model2().to(device)
        model_path = './trained_models/cifar10_f2_exp1_CIFAR10_0.5_True_dif_model.pth'
        elicitor = ResNet34().to(device)
        elicitor_path = './trained_models/cifar10_f1_exp1_CIFAR10_0.5_True_dif_model.pth'
    ...
    s1 = []
    s2 = []
    exp_mark = f"CIFAR10_no_verification_agent{agent_number}_{noise_type}"
    for i in range(len(noise_rate_list)):
        noise_rate = noise_rate_list[i]
        score_zero_one_peer_one_hot, score_ce_peer_prob= calculate_score(noise_rate, noise_type, model, elicitor, test_dataloader, Loss_zero_one_peer_one_hot,Loss_ce_peer_prob)
    ...
    df.to_csv(f"./CIFAR10/no_verification_noise_type_{noise_type}_agent{agent_number}.csv", index = False)
